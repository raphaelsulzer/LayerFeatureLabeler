...
def getLastFeature(layer):

    # gets the last feature of a layer

    req = QgsFeatureRequest()

    req.setFilterFid(layer.allFeatureIds()[-1])

    feat = layer.getFeatures(req).next()
...
def closeFeatures(layer, mapPoint):
    layerData = []
    ...
    for f in layer.getFeatures():
        ...
        info = (closestFeatureId, shortestDistance)
    ...
    #layerWithClosestFeature1, closestFeatureId1, shortestDistance1 = layerData[1]
    #layerWithClosestFeature2, closestFeatureId2, shortestDistance2 = layerData[2]


    return closestFeatureId
    #return [closestFeatureId, closestFeatureId1, closestFeatureId2]





def calculateBuffer(iface, layer, origins, cutoff_distance):
    ...
    if origins > 0:
        ...
        buffer_layer = getLegendLayerByName(iface, "Buffers")
        # create one if it doesn't exist
        if not buffer_layer:
            attribs = ['id', 'distance']
            types = [QtCore.QVariant.String, QtCore.QVariant.Double]
            buffer_layer = createTempLayer('Buffers', 'POLYGON', layer.crs().postgisSrid(), attribs, types)
            loadTempLayer(buffer_layer)
        ...
        insertTempFeatures(buffer_layer, geoms, values)


def calculateIntersection(iface, cutter, layer):
    # use the buffer to cut from another layer
    #cutter = getLegendLayerByName(self.iface, "Buffers")
    ...
    if cutter.featureCount() > 0:
        ...
        intersection_layer = getLegendLayerByName(iface, "Intersection")
        ...
        dissolved_layer = getLegendLayerByName(iface, "Dissolved")
        ...
        addFields(dissolved_layer, ["area"], [QtCore.QVariant.Double])
